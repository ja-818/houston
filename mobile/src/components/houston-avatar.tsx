/**
 * Shared Houston helmet avatar + color resolution.
 *
 * Extracted from chat-view.tsx / conversation-row.tsx so agent picker rows,
 * list rows, and the chat header all render the same visual identity.
 */
...
/** Resolve a stored color token ("charcoal", etc.) or raw hex to a hex string. */
export function resolveAgentColor(stored?: string): string {
  if (!stored) return "#1a1a1a";
  return AGENT_COLORS[stored] ?? stored;
}

interface HoustonAvatarProps {
  /** Color token or hex. Optional — defaults to charcoal. */
  color?: string;
  /** Helmet glyph size in px. */
  glyphSize?: number;
  /** Outer circle size in px — defaults to a comfortable 40. */
  ringSize?: number;
  className?: string;
}

/**
 * Circular Houston-branded avatar. Renders the helmet mark on a translucent
 * tinted disc. The caller passes the agent's color token and a size.
 */
export function HoustonAvatar({
  ...
  return (
    ...
    >
      <HoustonHelmet color={resolved} size={glyphSize} />
    </div>
  );
}

/** Raw Houston helmet SVG — same path data as desktop agent-avatar.tsx. */
