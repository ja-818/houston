//! Read/write operations for `.keel/skills/*.md` files.
//!
//! Each skill is a markdown file with `## Instructions` and `## Learnings` sections.

use super::helpers::{ensure_keel_dir, keel_dir};
use super::types::Skill;
use std::fs;
use std::path::Path;

/// List all skills by scanning `.keel/skills/*.md`.
pub fn list(root: &Path) -> Result<Vec<Skill>, String> {
    let dir = keel_dir(root).join("skills");
    if !dir.exists() {
        return Ok(Vec::new());
    }
...
pub fn read(root: &Path, name: &str) -> Result<Skill, String> {
    let path = keel_dir(root).join("skills").join(format!("{name}.md"));
    if !path.exists() {
